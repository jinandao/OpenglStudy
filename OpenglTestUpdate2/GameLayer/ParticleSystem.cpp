...
bool ParticleSystem::InitParticleSystem(const Vector3f& Pos)
{
	...
	m_billboardTechnique->SetBillboardSize(1);

	m_pTexture = new Texture("Content/fireworks_red.jpg");
	
	return glGetError() == GL_NO_ERROR;
}

void ParticleSystem::SetTechnique(PSUpdateTechnique* ps, PSBillboardTechnique* pb)
{
	m_updateTechnique = ps;
	m_billboardTechnique = pb;
}
...
{
	if (m_updateTechnique == nullptr)
	{
		std::cout << "updatetechnique null" << std::endl;
	}
...
{
	if (m_billboardTechnique == nullptr)
	{
		std::cout << "updatetechnique null" << std::endl;
	}
	m_billboardTechnique->Enable();
